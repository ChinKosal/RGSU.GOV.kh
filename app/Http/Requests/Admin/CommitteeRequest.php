...
class CommitteeRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'category'          => 'required',
            'content_km'        => request('type') == config('dummy.committee.term_of_reference.key') ? 'required' : '',
            'content_en'        => request('type') == config('dummy.committee.term_of_reference.key') ? 'required' : '',
            'file'              => request('type') == config('dummy.committee.member_list.key') ? 'required' : '',
        ];
    }
