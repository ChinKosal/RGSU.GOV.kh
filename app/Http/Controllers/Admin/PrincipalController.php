...
use App\Http\Requests\Admin\PrincipalRequest;
...
class PrincipalController extends Controller
{
    ...
    {
        $this->middleware('permission:principal-recipient-view', ['only' => ['index', 'data', 'page']]);
        $this->middleware('permission:principal-recipient-create', ['only' => ['onStore']]);
        $this->middleware('permission:principal-recipient-update', ['only' => ['bulkHideShow', 'saveSingleOption']]);
        $this->middleware('permission:principal-recipient-delete', ['only' => ['onRestore','onDelete','onDestroy']]);

        $this->active   = config('dummy.status.active.key');
        $this->disabled = config('dummy.status.disabled.key');
        $this->model    = Principal::class;
    }
    ...
    public function onStore(PrincipalRequest $request)
    {
        $isTypePrincipalPudr = $request->type == config('dummy.principal_recipient.pudr.key');
        $content = $isTypePrincipalPudr ? null : json_encode([
