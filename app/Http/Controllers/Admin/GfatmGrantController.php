...
use App\Http\Requests\Admin\GfatmGrantRequest;
use App\Models\GfatmGrant;
use App\Services\QueryService;
use Illuminate\Http\Request;

class GfatmGrantController extends Controller
{
    protected $active, $disabled, $model;
    public function __construct()
    {
        $this->middleware('permission:gfatm-grant-view', ['only' => ['index', 'data', 'page']]);
        $this->middleware('permission:gfatm-grant-create', ['only' => ['onStore']]);
        $this->middleware('permission:gfatm-grant-update', ['only' => ['bulkHideShow', 'saveSingleOption']]);
        $this->middleware('permission:gfatm-grant-delete', ['only' => ['onRestore','onDelete','onDestroy']]);

        $this->active   = config('dummy.status.active.key');
        $this->disabled = config('dummy.status.disabled.key');
        $this->model    = GfatmGrant::class;
    }

    public function index()
    {
        return view("admin::pages.gfatm-grant.index");
    }
    ...
    public function onStore(GfatmGrantRequest $request)
    {
        $items = [
            'title'         => json_encode([
                'en' => $request->title_en ?? '',
