<section class="container-full w-full h-auto mt-4">
    <div class="container-section w-[60%] m-auto h-auto">
        <div class="flex w-full h-[370px] drop-shadow-md bg-white">
            <div class="box-image w-[45%] h-full">
                <a href="#"><img class="w-full h-full object-cover"
                        src="https://static.independent.co.uk/2023/08/22/09/AFP_33RE4VU.jpg" alt=""></a>
            </div>
            <div class="box-description w-[55%] h-full px-4 py-2 overflow-hidden">
                <p class="font-Poppins text-lg font-medium break-all">Hun Manet is a prominent Cambodian military
                    officer
                    and
                    politician,
                    born on
                    August
                    1, 1977. As the
                    eldest son of Prime Minister Hun Sen, Hun Manet has been a key figure in Cambodian politics and
                    the
                    military. He holds the rank of Lieutenant General in the Royal Cambodian Armed Forces (RCAF) and
                    has
                    been involved in various capacities within the military structure.
                    Hun Manet has played a role in fostering relationships between Cambodia and other countries, and
                    he
                    has been actively involved in promoting diplomatic ties. Additionally, he has been associated
                    with
                    charitable and social initiatives. His position within the military has been seen by some as a
                    strategic move, potentially positioning him for a more prominent role in Cambodian politics in
                    the
                    future.</p>
            </div>
        </div>

        <div class="speeches-section h-[50px] mt-4 relative" style="border-bottom: 4px solid #003399">
            <div class="speeches w-fit h-full flex items-center  absolute  left-0 top-0">
                <p class="text-lg font-Hanuman">Speeches</p>
            </div>
        </div>

        <div class="card-section w-full h-auto mt-7 grid grid-cols-3 gap-5">
            <div class="card w-full h-[350px] drop-shadow-sm bg-white">
                <div class="box-image w-full h-[55%] ">
                    <a href="#"> <img class="w-full h-full object-cover" src="./images/cards/card_img2.jpg"
                            alt=""></a>
                </div>
                <div class="description w-full h-[45%] p-3">
                    <a href="#"
                        class="text-base font-Poppins font-medium break-all line-clamp-4 text-black decoration-transparent hover:text-blue-800">
                        On the occasion of the
                        meeting,
                        Mr. Kojima
                        Masanobu thanked the President for
                        taking the time to
                        allow him and his colleagues to pay a courtesy call and to inform about the progress of
                        training
                        and bring Cambodian trainees to work. In Japan.
                    </a>
                    <div class="date flex items-center gap-1 mt-2 text-sm opacity-85">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M5.615 21q-.69 0-1.152-.462Q4 20.075 4 19.385V6.615q0-.69.463-1.152Q4.925 5 5.615 5h1.77V3.308q0-.233.153-.386q.152-.153.385-.153t.386.153q.153.153.153.386V5h7.153V3.27q0-.214.144-.357t.356-.144q.214 0 .357.143t.143.357V5h1.77q.69 0 1.152.463q.463.462.463 1.152v12.77q0 .69-.462 1.152q-.463.463-1.153.463zm0-1h12.77q.23 0 .423-.192q.192-.193.192-.423v-8.77H5v8.77q0 .23.192.423q.193.192.423.192M5 9.615h14v-3q0-.23-.192-.423Q18.615 6 18.385 6H5.615q-.23 0-.423.192Q5 6.385 5 6.615zm0 0V6zm7 4.539q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23M12 18q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q12.31 18 12 18m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q8.31 18 8 18m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q16.31 18 16 18" />
                        </svg>
                        <span class="">20/October/2022</span>
                    </div>
                </div>
            </div>
            <div class="card w-full h-[350px] drop-shadow-sm bg-white">
                <div class="box-image w-full h-[55%] ">
                    <a href="#"> <img class="w-full h-full object-cover" src="./images/cards/card_img3.jpg"
                            alt=""></a>
                </div>
                <div class="description w-full h-[45%] p-3">
                    <a href="#"
                        class="text-base font-Poppins font-medium break-all line-clamp-4 text-black decoration-transparent hover:text-blue-800">
                        On the occasion of the
                        meeting,
                        Mr. Kojima
                        Masanobu thanked the President for
                        taking the time to
                        allow him and his colleagues to pay a courtesy call and to inform about the progress of
                        training
                        and bring Cambodian trainees to work. In Japan.
                    </a>
                    <div class="date flex items-center gap-1 mt-2 text-sm opacity-85">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M5.615 21q-.69 0-1.152-.462Q4 20.075 4 19.385V6.615q0-.69.463-1.152Q4.925 5 5.615 5h1.77V3.308q0-.233.153-.386q.152-.153.385-.153t.386.153q.153.153.153.386V5h7.153V3.27q0-.214.144-.357t.356-.144q.214 0 .357.143t.143.357V5h1.77q.69 0 1.152.463q.463.462.463 1.152v12.77q0 .69-.462 1.152q-.463.463-1.153.463zm0-1h12.77q.23 0 .423-.192q.192-.193.192-.423v-8.77H5v8.77q0 .23.192.423q.193.192.423.192M5 9.615h14v-3q0-.23-.192-.423Q18.615 6 18.385 6H5.615q-.23 0-.423.192Q5 6.385 5 6.615zm0 0V6zm7 4.539q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23M12 18q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q12.31 18 12 18m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q8.31 18 8 18m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q16.31 18 16 18" />
                        </svg>
                        <span class="">20/October/2022</span>
                    </div>
                </div>
            </div>
            <div class="card w-full h-[350px] drop-shadow-sm bg-white">
                <div class="box-image w-full h-[55%] ">
                    <a href="#"> <img class="w-full h-full object-cover" src="./images/cards/card_img4.jpg"
                            alt=""></a>
                </div>
                <div class="description w-full h-[45%] p-3">
                    <a href="#"
                        class="text-base font-Poppins font-medium break-all line-clamp-4 text-black decoration-transparent hover:text-blue-800">
                        On the occasion of the
                        meeting,
                        Mr. Kojima
                        Masanobu thanked the President for
                        taking the time to
                        allow him and his colleagues to pay a courtesy call and to inform about the progress of
                        training
                        and bring Cambodian trainees to work. In Japan.
                    </a>
                    <div class="date flex items-center gap-1 mt-2 text-sm opacity-85">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M5.615 21q-.69 0-1.152-.462Q4 20.075 4 19.385V6.615q0-.69.463-1.152Q4.925 5 5.615 5h1.77V3.308q0-.233.153-.386q.152-.153.385-.153t.386.153q.153.153.153.386V5h7.153V3.27q0-.214.144-.357t.356-.144q.214 0 .357.143t.143.357V5h1.77q.69 0 1.152.463q.463.462.463 1.152v12.77q0 .69-.462 1.152q-.463.463-1.153.463zm0-1h12.77q.23 0 .423-.192q.192-.193.192-.423v-8.77H5v8.77q0 .23.192.423q.193.192.423.192M5 9.615h14v-3q0-.23-.192-.423Q18.615 6 18.385 6H5.615q-.23 0-.423.192Q5 6.385 5 6.615zm0 0V6zm7 4.539q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23M12 18q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q12.31 18 12 18m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q8.31 18 8 18m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q16.31 18 16 18" />
                        </svg>
                        <span class="">20/October/2022</span>
                    </div>
                </div>
            </div>
            <div class="card w-full h-[350px] drop-shadow-sm bg-white">
                <div class="box-image w-full h-[55%] ">
                    <a href="#"> <img class="w-full h-full object-cover" src="./images/cards/card_img5.jpg"
                            alt=""></a>
                </div>
                <div class="description w-full h-[45%] p-3">
                    <a href="#"
                        class="text-base font-Poppins font-medium break-all line-clamp-4 text-black decoration-transparent hover:text-blue-800">
                        On the occasion of the
                        meeting,
                        Mr. Kojima
                        Masanobu thanked the President for
                        taking the time to
                        allow him and his colleagues to pay a courtesy call and to inform about the progress of
                        training
                        and bring Cambodian trainees to work. In Japan.
                    </a>
                    <div class="date flex items-center gap-1 mt-2 text-sm opacity-85">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M5.615 21q-.69 0-1.152-.462Q4 20.075 4 19.385V6.615q0-.69.463-1.152Q4.925 5 5.615 5h1.77V3.308q0-.233.153-.386q.152-.153.385-.153t.386.153q.153.153.153.386V5h7.153V3.27q0-.214.144-.357t.356-.144q.214 0 .357.143t.143.357V5h1.77q.69 0 1.152.463q.463.462.463 1.152v12.77q0 .69-.462 1.152q-.463.463-1.153.463zm0-1h12.77q.23 0 .423-.192q.192-.193.192-.423v-8.77H5v8.77q0 .23.192.423q.193.192.423.192M5 9.615h14v-3q0-.23-.192-.423Q18.615 6 18.385 6H5.615q-.23 0-.423.192Q5 6.385 5 6.615zm0 0V6zm7 4.539q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23M12 18q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q12.31 18 12 18m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q8.31 18 8 18m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q16.31 18 16 18" />
                        </svg>
                        <span class="">20/October/2022</span>
                    </div>
                </div>
            </div>
            <div class="card w-full h-[350px] drop-shadow-sm bg-white">
                <div class="box-image w-full h-[55%] ">
                    <a href="#"> <img class="w-full h-full object-cover" src="./images/cards/card_img6.jpg"
                            alt=""></a>
                </div>
                <div class="description w-full h-[45%] p-3">
                    <a href="#"
                        class="text-base font-Poppins font-medium break-all line-clamp-4 text-black decoration-transparent hover:text-blue-800">
                        On the occasion of the
                        meeting,
                        Mr. Kojima
                        Masanobu thanked the President for
                        taking the time to
                        allow him and his colleagues to pay a courtesy call and to inform about the progress of
                        training
                        and bring Cambodian trainees to work. In Japan.
                    </a>
                    <div class="date flex items-center gap-1 mt-2 text-sm opacity-85">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M5.615 21q-.69 0-1.152-.462Q4 20.075 4 19.385V6.615q0-.69.463-1.152Q4.925 5 5.615 5h1.77V3.308q0-.233.153-.386q.152-.153.385-.153t.386.153q.153.153.153.386V5h7.153V3.27q0-.214.144-.357t.356-.144q.214 0 .357.143t.143.357V5h1.77q.69 0 1.152.463q.463.462.463 1.152v12.77q0 .69-.462 1.152q-.463.463-1.153.463zm0-1h12.77q.23 0 .423-.192q.192-.193.192-.423v-8.77H5v8.77q0 .23.192.423q.193.192.423.192M5 9.615h14v-3q0-.23-.192-.423Q18.615 6 18.385 6H5.615q-.23 0-.423.192Q5 6.385 5 6.615zm0 0V6zm7 4.539q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23M12 18q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q12.31 18 12 18m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q8.31 18 8 18m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q16.31 18 16 18" />
                        </svg>
                        <span class="">20/October/2022</span>
                    </div>
                </div>
            </div>
            <div class="card w-full h-[350px] drop-shadow-sm bg-white">
                <div class="box-image w-full h-[55%] ">
                    <a href="#"> <img class="w-full h-full object-cover" src="./images/cards/card_img3.jpg"
                            alt=""></a>
                </div>
                <div class="description w-full h-[45%] p-3">
                    <a href="#"
                        class="text-base font-Poppins font-medium break-all line-clamp-4 text-black decoration-transparent hover:text-blue-800">
                        On the occasion of the
                        meeting,
                        Mr. Kojima
                        Masanobu thanked the President for
                        taking the time to
                        allow him and his colleagues to pay a courtesy call and to inform about the progress of
                        training
                        and bring Cambodian trainees to work. In Japan.
                    </a>
                    <div class="date flex items-center gap-1 mt-2 text-sm opacity-85">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M5.615 21q-.69 0-1.152-.462Q4 20.075 4 19.385V6.615q0-.69.463-1.152Q4.925 5 5.615 5h1.77V3.308q0-.233.153-.386q.152-.153.385-.153t.386.153q.153.153.153.386V5h7.153V3.27q0-.214.144-.357t.356-.144q.214 0 .357.143t.143.357V5h1.77q.69 0 1.152.463q.463.462.463 1.152v12.77q0 .69-.462 1.152q-.463.463-1.153.463zm0-1h12.77q.23 0 .423-.192q.192-.193.192-.423v-8.77H5v8.77q0 .23.192.423q.193.192.423.192M5 9.615h14v-3q0-.23-.192-.423Q18.615 6 18.385 6H5.615q-.23 0-.423.192Q5 6.385 5 6.615zm0 0V6zm7 4.539q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.539q-.23.23-.54.23M12 18q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q12.31 18 12 18m-4 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q8.31 18 8 18m8 0q-.31 0-.54-.23q-.23-.23-.23-.54q0-.309.23-.539q.23-.23.54-.23q.31 0 .54.23q.23.23.23.54q0 .31-.23.54Q16.31 18 16 18" />
                        </svg>
                        <span class="">20/October/2022</span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
