...
@section('layout')
    <div class="content-wrapper" x-data="page">
        @include('admin::shared.header', [
            'title' => __('form.title.activity'),
            'header_name' => __('form.name.activity'),
        ])
        <div class="content-body">
            <div class="content-tab">
                <div class="content-tab-wrapper">
                    <span class="title">
                        @lang('form.total', ['name' => __('form.title.activity')]) <span x-text="table?.paginate?.totalItems"></span>
                    </span>
                </div>
            ...
            @include('admin::pages.activity.table')
        </div>
        @include('admin::pages.activity.store')
    </div>
@stop
@section('script')
    ...
    <script type="module">
        Alpine.data('page', () => ({
            table: new Table("{{ route('admin-activity-data') }}"),
            Ids: [],
            status: false,
            ...
            onUpdateOption(data, option){
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-activity-saveSingleOption') }}`,
                                method: 'POST',
                                data: {
                                    id: data.id,
                                    option: option
            ...
            bulkHideShow(option) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: "{{ route('admin-activity-bulk-hide-show') }}",
                                method: 'DELETE',
            ...
            onRestore(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-activity-restore') }}`,
                                method: 'PUT',
                                data: {
                                    id: data.id
                                }
