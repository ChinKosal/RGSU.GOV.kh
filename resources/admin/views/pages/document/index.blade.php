...
@section('layout')
    <div class="content-wrapper" x-data="page">
        ...
        <div class="content-body">
            ...
            @include('admin::pages.document.table')
        </div>
        @include('admin::pages.document.store')
    </div>
@stop
@section('script')
    <script type="module">
        Alpine.data('page', () => ({
            table: new Table("{{ route('admin-document-data') }}"),
            Ids: [],
            status: false,
            ...
            onUpdateOption(data, option){
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-document-saveSingleOption') }}`,
                                method: 'POST',
                                data: {
                                    id: data.id,
                                    option: option
            ...
            bulkHideShow(option) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: "{{ route('admin-document-bulk-hide-show') }}",
                                method: 'DELETE',
            ...
            onRestore(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-document-restore') }}`,
                                method: 'PUT',
                                data: {
                                    id: data.id
                                }
