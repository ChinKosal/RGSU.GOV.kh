    ...
    <div x-data="storeDialog" class="form-admin">
        <form class="form-wrapper scroll-form">
            <div class="form-header">
                <h3 x-show="!dialogData?.id">@lang('form.header.create', ['name' => __('form.title.social_media')])</h3>
                <h3 x-show="dialogData?.id">@lang('form.header.update', ['name' => __('form.title.social_media')])</h3>
                    ...
                    <div class="row-2">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-select-photo image" @click="selectImage(event)">
                                <div class="select-photo" :class={active:form?.icon}>
                                    <div class="icon">
                                ...
                                <template x-if="form?.icon">
                                    <div class="image-view active">
                                        <img x-bind:src="baseImageUrl + form?.icon" alt="" class="!object-contain">
                                    </div>
                                </template>
                                <input type="hidden" x-model="form.icon" autocomplete="off" role="presentation">
                            </div>
                            <span class="error" x-show="validate?.icon" x-text="validate?.icon"></span>
                        </div>
                    </div>
                    <div class="row">
                ...
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            this.form.disable();
                            this.loading = true;
                            this.form.type = @json(request()->type);
                            const data = this.form.value();
                            Axios({
                                url: `{{ route('admin-social-media-save') }}`,
                                method: 'POST',
                                data: {
                                    ...data,
