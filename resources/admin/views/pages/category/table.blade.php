<div class="table">
    ...
    <template x-if="!table.loading && !table?.empty()">
        <div class="table-wrapper">
            <div class="table-header">
                ...
                <div class="row table-row-30">
                    <span>@lang('table.field.name_en')</span>
                </div>
            ...
            <div class="table-body">
                <template x-for="item in table.data">
                    <div class="column" x-data="{ status: item.status }">
                        ...
                        <div class="row table-row-30">
                            <span x-text="JSON.parse(item.name).km || '---'"></span>
                        </div>
                        <div class="row table-row-30">
                            <span x-text="JSON.parse(item.name).en || '---'"></span>
                        </div>
                        <div class="row table-row-15">
    ...
    <template x-if="table && table?.empty()">
        @component('admin::components.empty', [
            'name' => __('table.empty.title', ['name' => __('form.title.category.' . request('type'))]),
            'msg' => __('table.empty.message', ['name' => __('form.title.category.' . request('type'))]),
        ])
        @endcomponent
    </template>
</div>
